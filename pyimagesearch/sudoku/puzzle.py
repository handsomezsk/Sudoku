...
def CN_extract_digit(cell, debug=True):
	if debug:
	...
	# check to see if we are visualizing the cell thresholding step
	if debug:
		cv2.namedWindow('Cleared', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
    ...
    if debug:
        cv2.namedWindow('Dilated', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
        cv2.imshow("Dilated", thresh)
        cv2.waitKey(0)'''

	(h, w) = thresh.shape
	percentFilled = cv2.countNonZero(thresh) / float(w * h)
	if percentFilled <= 0.035:
		if debug:
			print("None")
		return None
	...
	# check to see if we should visualize the masking step
	if debug:
		cv2.namedWindow('Digit', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)
