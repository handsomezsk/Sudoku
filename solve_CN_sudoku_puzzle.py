...
from pyimagesearch.sudoku import CN_extract_digit,extract_digit
from pyimagesearch.sudoku import find_puzzle
from tensorflow.keras.preprocessing.image import img_to_array
from tensorflow.keras.models import load_model
from sudoku import Sudoku
from solve import return_solve, print_sudoku
import numpy as np
import argparse
import imutils
import cv2
...
for y in range(0, 9):
	...
	for x in range(0, 9):
		...
		digit = CN_extract_digit(cell, debug=args["debug"] > 0)
...
arr=board.tolist()
problem = copy.deepcopy(arr)
print("[INFO] solving sudoku puzzle...")
solution = return_solve(arr)

# solution=np.array(solution)
# solution.show_full()

# # loop over the cell locations and board
for (cellRow, problemRow, solutionRow) in zip(cellLocs, problem, solution):
	# loop over individual cell in the row
	for (box, init_digit, digit) in zip(cellRow, problemRow, solutionRow):
